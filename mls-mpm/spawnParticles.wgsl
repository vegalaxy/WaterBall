...
fn spawn() {
    ...
    for (var layer = 0; layer < radial_layers; layer++) {
        ...
        let current_radius = tube_radius * radius_fraction * 0.8;
        ...
        for (var angle_idx = 0; angle_idx < particles_per_layer; angle_idx++) {
            ...
            let jitter_x = (fract(sin(f32(particle_idx) * 12.9898) * 43758.5453) - 0.5) * 1.0;
            let jitter_y = (fract(sin(f32(particle_idx) * 78.233) * 43758.5453) - 0.5) * 0.5;
            let jitter_z = (fract(sin(f32(particle_idx) * 45.164) * 43758.5453) - 0.5) * 0.5;
